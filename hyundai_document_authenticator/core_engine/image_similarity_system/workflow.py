...
def _initialize_provider_manager(
    ...
    Args:
        ...
        run_identifier (Optional[str]): Optional run identifier to derive a per-run embedded
            Qdrant location when configured (unique_location_per_run=true).
    ...
    # Local helpers to remove duplicated auto-build sequences while preserving logs.
    def _faiss_auto_build(warn_msg: str, err_msg: str) -> FaissIndexManager:
        """Attempt automatic FAISS index build and return a re-instantiated manager.

        Args:
            warn_msg (str): Exact warning message to log before attempting auto-build.
            err_msg (str): Exact error message to log if auto-build raises.

        Returns:
            FaissIndexManager: Newly instantiated FAISS manager after build attempt.
        """
        ...
        try:
            ...
            # Keep error string exact per call site to preserve logs.
            logger.error(err_msg, e_build, exc_info=True)
        # Recreate and return manager (retry happens at call site to preserve behavior).
    ...
    def _qdrant_auto_build(warn_msg: str) -> 'QdrantManager':
        """Attempt automatic Qdrant collection build and return a re-instantiated manager.

        Args:
            warn_msg (str): Exact warning message to log before attempting auto-build.

        Returns:
            QdrantManager: Newly instantiated Qdrant manager after build attempt.
        """
        logger.warning(warn_msg)
        try:
    ...
    if provider == 'faiss':
        ...
        try:
            ...
            if not loaded and build_index_on_load_failure:
                vdb = _faiss_auto_build(
                    "FAISS index load failed; attempting automatic index build as configured (build_index_on_load_failure=true).",
                    "Automatic index building failed: %s",
                )
                loaded = _try_load_faiss()
            ...
            if not loaded:
                ...
                if vdb.is_index_loaded_and_ready():
                    ...
                    if build_index_on_load_failure:
                        vdb = _faiss_auto_build(
                            "FAISS index not ready; attempting automatic index build as configured (build_index_on_load_failure=true).",
                            "Automatic index building (empty) failed: %s",
                        )
                        try:
    ...
    if provider == 'qdrant':
        ...
        try:
            ...
            # Allow optional per-run unique embedded location to avoid lock contention in dev/test.
            # Enabled when qdrant.unique_location_per_run=true and location is configured.
            qdrant_conf_effective = dict(qdrant_conf)
            try:
                if run_identifier and bool(qdrant_conf.get('unique_location_per_run', False)) and qdrant_conf.get('location'):
                    loc_val = qdrant_conf.get('location')
                    base_path = Path(str(loc_val))
                    if not base_path.is_absolute():
                        base_path = (project_root / base_path).resolve()
                    qdrant_conf_effective['location'] = str((base_path / run_identifier).resolve())
            except Exception:
                pass
            vdb = QdrantManager(
                feature_dim=feature_extractor.feature_dim,
                collection_name_stem=qdrant_conf.get('collection_name_stem', 'image_similarity_collection'),
                model_name=feature_extractor.model_name,
                qdrant_config=qdrant_conf_effective,
                project_root_path=project_root,
            )
            loaded_ok = False
            try:
                ...
                if build_index_on_load_failure:
                    ...
                    try:
                        if bool(qdrant_conf.get('force_recreate_collection', False)):
                            vdb.clear_index()
                        idx_params = full_config.get('indexing_task', {}) or {}
                        ...
                        logger.info(
                            "Qdrant auto index build result: status=%s, exit_code=%s",
                            ("success" if exit_code_build == 0 else "error"), exit_code_build
                        )
                        if vdb.load_index() and vdb.is_index_loaded_and_ready():
                            base_manager = vdb
                            indexed_item_count = vdb.get_total_indexed_items()
...
def _execute_tif_batch_search_with_augmentation(
    ...
    try:
        ...
        create_new_subfolder = bool(st.get('new_query_new_subfolder', False))
        save_outputs_to_folder = bool(st.get('save_outputs_to_folder', False))
        ...
        # -------------------------
        # Stream augmentation per parent to avoid large in-memory concatenations
        ...
        for parent in store.iter_parents():
            ...
            if vecs.size == 0:
                continue
            try:
                orch.augment_with_batch(vecs.astype(np.float32, copy=False), names)
                total_augmented += int(vecs.shape[0])
            except Exception as e_aug:
                # When persistent augmentation fails (e.g., IVF governance), surface error as before
                logger.error("Augmentation failed: %s", e_aug)
                return {"status": "error", "exit_code": 1, "message": str(e_aug)}

        if total_augmented == 0:
            # No query vectors exist; Phase B (including bruteforce) has nothing to evaluate by design.
        ...
        generate_previews = bool(st.get('generate_tif_previews', False))
        create_per_query_subfolders = bool(st.get('create_per_query_subfolders_for_tif', False))
        save_search_summary_json = bool(st.get('save_search_summary_json', False))
        ...
        for tif_path in tqdm.tqdm(query_tif_files, desc="Phase B: searching (TIFs)"):
            ...
            if save_outputs_to_folder and st.get('copy_query_image_to_output', False):
                try:
            ...
            if save_outputs_to_folder and st.get('copy_similar_images_to_output', False) and top_docs_for_query:
        ...
        if save_outputs_to_folder and st.get('save_search_summary_json', False):
            global_section = {
                "aggregation_strategy_used": aggregation_strategy,
                "top_k_used": top_k,
                "top_doc_used": top_doc,
        ...
        try:
            if cloned_config.get('search_task', {}).get('save_results_to_postgresql', True):
