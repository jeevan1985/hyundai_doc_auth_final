...
configure_qdrant() {
    ...
    if [ "$QDRANT_MODE" = "server" ]; then
        ...
        echo "   -> Updating YAML via config_env_merge (server mode)..."
        python -m hyundai_document_authenticator.extra_tools.config_env_merge qdrant \
            --mode "$QDRANT_MODE" --host "$QDRANT_HOST" --port "$QDRANT_PORT" \
            --file "$API_CONFIG_FILE" --file "$CLI_CONFIG_FILE"
        echo -e "${C_GREEN}   ✓ Configured for Server Mode (YAML updated).${C_RESET}"
    else
        echo "   -> Updating YAML via config_env_merge (embedded mode)..."
        python -m hyundai_document_authenticator.extra_tools.config_env_merge qdrant \
            --mode "$QDRANT_MODE" --host "$QDRANT_HOST" --port "$QDRANT_PORT" \
            --file "$API_CONFIG_FILE" --file "$CLI_CONFIG_FILE"
        echo -e "${C_GREEN}   ✓ Configured for Embedded Mode (YAML updated).${C_RESET}"
    fi
}
...
configure_database() {
    ...
    echo "   -> Updating YAML via config_env_merge (database URIs)..."
    python -m hyundai_document_authenticator.extra_tools.config_env_merge database \
        --file "$API_CONFIG_FILE"

    echo -e "${C_GREEN}   ✓ Database URIs configured for PostgreSQL (YAML updated).${C_RESET}"
}
