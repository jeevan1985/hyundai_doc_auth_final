...
configure_qdrant() {
    ...
    if [ "$QDRANT_MODE" = "server" ]; then
        ...
        echo "   -> Enabling network settings for Server Mode using 'sed'..."
        ...
        sed -i -E 's/^(\s*)#\s*(host:.*)/\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
        sed -i -E 's/^(\s*)#\s*(port:.*)/\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
        # Comment out local file setting
        sed -i -E 's/^(\s*)(location:.*)/#\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
        echo -e "${C_GREEN}   ✓ Configured for Server Mode.${C_RESET}"
    else
        echo "   -> Enabling local file settings for Embedded Mode..."
        # This is the reverse logic: comment out server settings and uncomment the local one.
        sed -i -E 's/^(\s*)(host:.*)/#\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
        sed -i -E 's/^(\s*)(port:.*)/#\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
        sed -i -E 's/^(\s*)#\s*(location:.*)/\1\2/' "$API_CONFIG_FILE" "$CLI_CONFIG_FILE"
...
configure_database() {
    ...
    echo "   -> Setting database URIs in config file..."

    # Construct the database connection URIs using environment variables.
    local RESULTS_DB_URI="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${POSTGRES_DB}"
    local USER_DB_URI="postgresql://${POSTGRES_USER_USER}:${POSTGRES_USER_PASSWORD}@${POSTGRES_USER_HOST}:${POSTGRES_USER_PORT:-5432}/${POSTGRES_USER_DB}"

    # Use 'sed' to robustly replace the database_uri lines.
    # We use '|' as the separator instead of '/' to avoid conflicts with slashes in the URI.
    # The `(#\s*)?` part of the regex makes the leading comment optional. This means
    # the command will correctly replace the line whether it is currently commented out or not.
    sed -i -E "s|^(#\s*)?database_uri:.*|database_uri: ${RESULTS_DB_URI}|" "$API_CONFIG_FILE"
    sed -i -E "s|^(#\s*)?user_database_uri:.*|user_database_uri: ${USER_DB_URI}|" "$API_CONFIG_FILE"

    echo -e "${C_GREEN}   ✓ Database URIs configured for PostgreSQL.${C_RESET}"
}
